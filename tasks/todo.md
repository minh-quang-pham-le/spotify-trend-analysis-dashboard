...
- [~] **D3. Page: Temporal Trends — feature evolution + explicit share.**
  ...
  - ⚠ **OPEN MODELING DECISION (blocks build):** `release_date` is on `dim_album`, features/`explicit` on `dim_track`; single-direction relationships prevent grouping one dim's measure by another's attribute. **Approach B (recommended):** denormalize `release_year` + `release_quarter` onto `dim_track` in the pipeline (TDD change to `build_dim_track`; re-run `make build` + Refresh; reuses existing measures; also unblocks D5). **Approach A (fallback):** `dim_album` calc column + ~5 new SUMMARIZE measures, no pipeline change. Awaiting team pick before implementing.
