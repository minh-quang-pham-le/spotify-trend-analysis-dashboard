...
- [~] **D3. Page: Temporal Trends — feature evolution + explicit share.**
  ...
  - **Built & verified (2026-06-01):** screenshots `d3_feature_evolution_line.png` + `d3_explicit_share_area.png` inspected — correct `release_year` axis, all 4 measures, 2025 endpoints match. Explicit chart built as a single `% Explicit` area (leaner variant; Chart 5 doc aligned). ⚠ **One fix pending before complete:** apply the `release_year ≥ 2010` visual filter (charts currently show the noisy 1900–2025 range) → re-export → re-verify.
...
- [~] **D5. Page: Audio Anatomy — correlation heatmap + group box plot.**
  ...
  - Status (2026-06-01): **groundwork done** (no Power BI Desktop yet). 9×9 Pearson matrix computed (verified diagonal=1.0, symmetric). Justification = `report/chart_justifications.md` Charts 7 & 8; build steps + full matrix for verification = `powerbi/phase_d_build.md` D5.
    - **Part A — heatmap: BUILT, verified, two display fixes pending.** Screenshot `d5_audio_correlation_heatmap.png` inspected — values/diagonal(=1.0)/symmetry all correct (energy↔loudness 0.70, energy↔acousticness −0.50). ⚠ Pending before complete: (1) **center the diverging color scale at 0** (Min −1 red / Center 0 white / Max +1 blue — currently the white midpoint sits ~0.25, so weak positives read reddish); (2) **turn off the Total row/column** (it sums correlations — meaningless). Re-export → re-verify.
    - **Part B — box plot: feature/buckets chosen; needs a box-plot visual.** ✅ Feature = **acousticness** (median 0.25→0.17, IQR 0.47→0.32 — falls *and* narrows; cleanest dispersion story; alt = speechiness, widening). Buckets = `≤2019 / 2020-22 / 2023 / 2024 / 2025` (all ≥1,630 tracks). ⚠ Still needs a **non-native box-plot visual** (AppSource "Box & Whisker" or Python). Spec + verification table in `phase_d_build.md` D5 Part B.
