...
2. **No `daily_streams`.** Source has `daily_movement` / `weekly_movement` (chart-position deltas) but no stream counts. The Mood Map's *size = streams* encoding (SPEC §8 chart 3) needs a substitute — proposal: *size = popularity*.
