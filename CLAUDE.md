...
- Phase: **Phase D (dashboard pages) IN PROGRESS.** Done & screenshot-verified: **D1** Overview popularity histogram, **D2** Mood Map scatter, **D4** Artists top-20 bar. Groundwork + enablers done, awaiting Power BI build: **D3** Temporal Trends (line + explicit-share area — grain = release YEAR; modeling resolved via the pipeline change below) and **D5** Audio Anatomy (heatmap reads the new `corr_audio_features.csv`; box plot release-side unblocked, still needs a non-native box-plot visual). Per-chart build runbook = `powerbi/phase_d_build.md`; justifications = `report/chart_justifications.md`; grain/metric decisions + findings logged in `tasks/todo.md`. Not started: **D6** Geography, **D7** radar. (Phase C complete — Gate G4 PASSED; see `tasks/phase_c_summary.md`.)
- Pipeline: `ingest → clean → transform → validate → export` all implemented (tasks B1–B12, TDD). `make build` exits 0 on the 2025-06-11 snapshot — **2,110,287** fact rows; dims: dim_track=24,976, dim_artist=7,565, dim_album=17,229, dim_date=603 (2023-10-18→2025-06-11, contiguous), dim_country=73. **Phase-D enhancement (commit dd3cffb):** `dim_track` now also carries `release_year` + `release_quarter` (earliest album release, nullable — 5 NA), and the pipeline emits a **7th CSV** `corr_audio_features.csv` (9×9 audio-feature Pearson matrix, long form) for the D5 heatmap. **73 unit/contract tests green**, ruff check clean, coverage on clean/transform/validate ≥ 87%. (Note: pre-existing `ruff format` drift in transform.py/validate.py/test_transform.py — not introduced here; repo gate is `ruff check`.)
